...
def test_preemption():
    ...
    Scénario : Un échantillon STAT arrive pendant qu'un URGENT est en cours.
    Attendu : L'URGENT est interrompu, STAT traité, puis URGENT reprend.
    """
    print("\n" + "="*80)
    ...
    samples = [
        # URGENT commence à 08:00 (durée 60 min → fini à 09:00)
        {
            "id": "S001_URGENT",
            "type": "BLOOD",
            "priority": "URGENT",
        ...
        # STAT arrive à 08:30 → doit PRÉEMPTER S001
        {
            "id": "S002_STAT",
            "type": "BLOOD",
            "priority": "STAT",
    ...
    # Configuration avec préemption activée
...
def test_aging():
    ...
    Scénario : Des ROUTINE attendent longtemps pendant que des URGENT arrivent.
    Attendu : Les ROUTINE sont boostés en URGENT après seuil.
    """
    print("\n" + "="*80)
    ...
    samples = [
        # ROUTINE arrivé tôt (08:00) mais devrait attendre
        {
            "id": "S001_ROUTINE_OLD",
            "type": "BLOOD",
            "priority": "ROUTINE",
    ...
    technicians = [
        {"id": "TECH01", "speciality": "BLOOD", "available_from": 570}  # Disponible à 09:30
    ]
    ...
    # Configuration avec aging activé (seuil 60 min)
    ...
    print("\n📋 Résultat :")
    print("  (S001 devrait être traité AVANT S002/S003 grâce à l'aging)")
    for entry in result['schedule']:
        print(f"  {entry['sample_id']:20} : "
              f"{entry['start_time']:3}→{entry['end_time']:3} min "
              f"(priorité: {entry.get('sample_priority', 'N/A')})")
    
    print(f"\n📊 Métriques Aging :")
    if 'kernel_features' in result['metrics']:
        aging_stats = result['metrics']['kernel_features'].get('aging', {})
        print(f"  Échantillons aged : {aging_stats.get('total_aged_samples', 0)}")
        print(f"  Events d'aging : {aging_stats.get('aging_events', 0)}")
        config_info = aging_stats.get('config', {})
        print(f"  Seuil ROUTINE→URGENT : {config_info.get('routine_to_urgent_threshold', 'N/A')} min")
...
def test_load_balancing():
    ...
    Scénario : Plusieurs échantillons avec plusieurs techniciens disponibles.
    Attendu : Distribution équitable de la charge entre techniciens.
    """
    print("\n" + "="*80)
    ...
    # Configuration avec load balancing activé
    config = KernelSchedulerConfig(
        enable_preemption=False,
        enable_aging=False,
        enable_deadlock_detection=False,
    ...
    print("\n📋 Résultat (distribution par technicien) :")
    tech_assignments = {}
    for entry in result['schedule']:
    ...
    for tech_id, entries in tech_assignments.items():
        ...
        print(f"\n  {tech_id} : {len(entries)} échantillons, {total_duration} min total")
        for entry in entries:
            print(f"    - {entry['sample_id']} ({entry['duration']} min)")
    ...
    if 'kernel_features' in result['metrics']:
        ...
        print(f"  Écart-type : {lb_stats.get('std_deviation', 0):.1f} min")
        print(f"  Min/Max : {lb_stats.get('min_load', 0):.0f}/{lb_stats.get('max_load', 0):.0f} min")
        print(f"  Déséquilibre : {lb_stats.get('load_imbalance', 0):.0f} min")
...
def test_all_features_combined():
    """
    TEST 4 : TOUTES LES FEATURES COMBINÉES
    ...
    samples = [
        ...
        {"id": "S007_URGENT", "type": "CHEMISTRY", "priority": "URGENT", 
    ...
    technicians = [
        ...
        {"id": "TECH_CHEM_1", "speciality": "CHEMISTRY", "available_from": 540},
    ...
    equipment = [
        ...
        {"id": "EQUIP_CHEM_1", "type": "CHEMISTRY", "available_from": 540},
    ]
    ...
    if 'kernel_features' in result['metrics']:
        ...
        if 'preemption' in kf:
            print(f"    ⚠️ Préemptions : {kf['preemption']['total_preemptions']}")
        
        if 'aging' in kf:
            print(f"    🔼 Aging events : {kf['aging']['aging_events']}")
            print(f"       Échantillons boostés : {kf['aging']['total_aged_samples']}")
        
        if 'deadlock_detection' in kf:
            print(f"    💀 Deadlocks détectés : {kf['deadlock_detection']['total_deadlocks_detected']}")
        
        if 'load_balancing' in kf:
            lb = kf['load_balancing']
            print(f"    ⚖️ Coefficient de variation : {lb.get('coefficient_variation', 0):.1f}%")
